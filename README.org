...
Clone the Git repository into =~/.emacs.d=

#+BEGIN_SRC shell
$ git clone https://github.com/plexus/chemacs.git ~/.emacs.d
#+END_SRC

You can easily update Chemacs with a =git pull=. Chemacs is not available on
ELPA/MELPA because its special position in the Emacs boot process would cause a
